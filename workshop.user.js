...
// @downloadURL  https://raw.githubusercontent.com/ChrisRaven/EyeWire-Workshop/master/workshop.user.js
// ==/UserScript==

/*jshint esversion: 6 */
/*globals $, account, tomni, ColorUtils */ 
...
if (LOCAL) {
  ...
  serverPath = 'http://workshop.localhost/';
}
else {
...
(function() {
  ...
  let K = {
    ...
    XHR: function (args) {
      ...
      let successCallback = args.success || function () {};
      let errorCallback = args.error || function () {};

      let req = new XMLHttpRequest();
      if (args.withCredentials) {
        req.withCredentials = true;
      }
      req.open(method, args.url, true);
  ...
  function Workspace() {
    ...
    this.clear(); // to initialize the object with empty fields
  };
  ...
  class Workshop {
    ...
    addMenuEntry() {
      ...
      menuLink.innerHTML = '<a class="active">Workshop</a>';
      K.qS('#links > ul > li:nth-last-child(2)').before(menuLink);
    ...
    changeHeatMap(heatmap) {
      document.getElementById('highlightMethod').value = heatmap;
      K.gid('highlightMethod').dispatchEvent(new Event('change', {'bubbles': true}));
    }
    ...
    exitWorkshop() {
      ...
      if (this.unsaved) {
        var popup = new Attention.Confirmation({
          ...
          message: 'You have unsaved changes in the workspace. Do you want to quit?',
        ...
        popup.on('ok', function () {
          _this.exitWorkshop_confirm();
        });
      }
      else {
        this.exitWorkshop_confirm();
      }
    }

    changeUIVisibility(state) {
      let el;

      el = K.qS('.gameBoardUI'); if (el) { el.style.visibility = state; }
      el = K.gid('pageHeader'); if (el) { el.style.visibility = state; }
      el = K.gid('gameControls'); if (el) { el.style.visibility = state; }
      el = K.gid('scoutsLogFloatingControls'); if (el) { el.style.visibility = state; }
      el = K.gid('chatContainer'); if (el) { el.style.visibility = state; }
  
      // because of EWDLC "funStats"
      el = K.qS('.gameBar'); if (el) { el.style.display = state === 'hidden' ? 'none' : 'block'; }
    }
    ...
    addCell(id, exit = false) {
      let _this = this;

      if (this.cellsInUse[id] && !exit) {
        tomni.notificationManager.addChip({title: 'Cell ' + id + ' already exists'});
        return;
      }
      ...
      K.XHR({
        ...
        success: function (data) {
          ...
          let flags = '';
          flags += data.is_evil ? 'E' : '';
          flags += data.is_hunt ? 'H' : '';
        ...
        error: function () {
          tomni.notificationManager.addChip({title: 'Cell ' + id + ' not Found'});
        }
      });
    }
    ...
    removeCell(id) {
      ...
      delete this.cellsInUse[id];
      this.list.removeEl(id);
    }

    removeAll() {
      tomni.threeD.deleteCells();

      // source: https://stackoverflow.com/a/40953718
      for (const prop of Object.getOwnPropertyNames(this.cellsInUse)) {
        delete this.cellsInUse[prop];
      }

      this.list.removeAll();
      tomni.cell = this.originalCell;
      this.unsaved = false;
    }

    getColor(cellId) {
      if (this.cellsInUse[cellId]) {
        return this.cellsInUse[cellId].color;
      }

      return null;
    }

    changeColorHelper(cellId, color, blinking = false) {
      tomni.cell = cellId;
      if (color) {
        tomni.getCurrentCell().highlight({cubeids: this.cellsInUse[cellId].cubes, color: color, zindex: 1});
        if (!blinking) {
          this.cellsInUse[cellId].color = color;
        }
      }
      else {
        tomni.getCurrentCell().unhighlight(1);
        if (!blinking) {
          this.cellsInUse[cellId].color = null;
        }
      }
      tomni.getCurrentCell().update();
      tomni.cell = this.active;
    }

    changeCellColor(cellId, color, blinking = false, callback = function () {}) {
      let _this = this;

      if (this.cellsInUse[cellId].cubes) {
        this.changeColorHelper(cellId, color, blinking);
        callback();
      }
      else {
        K.XHR({
          url: '/1.0/cell/' + cellId + '/tasks',
          success: function () {
            _this.changeColorHelper(cellId, color, blinking);
            callback();
          }
        });
      }
  ...
  class ControlPanel {
    constructor (workshop) {
      ...
      this.node.innerHTML = `
        ...
        <label id="K_importFromFileWrapper">Import from file<input type="file" id="K_importFromFile" accept=".ewc"></label>
        <div id="K_browse">Browse</div>
        <div id="K_backToGame">Back to Game</div>
      `;

      document.body.appendChild(this.node);

      K.gid('K_addCellsButton').addEventListener('click', this.K_addCellsButton_clickListener.bind(this));
      K.gid('K_new').addEventListener('click', this.K_new_clickListener.bind(this));
      K.gid('K_save').addEventListener('click', this.K_save_clickListener.bind(this));
      K.gid('K_saveAs').addEventListener('click', this.K_saveAs_clickListener.bind(this));
      K.gid('K_importFromFile').addEventListener('change', this.K_importFromFile_changeListener.bind(this));
      K.gid('K_browse').addEventListener('click', this.K_browse_clickListener.bind(this));
      K.gid('K_backToGame').addEventListener('click', this.workshop.exitWorkshop.bind(this.workshop));
      document.addEventListener('K_cellLoaded', this.K_document_cellLoadedListener.bind(this));
    }
    ...
    K_addCellsButton_clickListener() {
      let popup = new AddPopup('Adding', 'Add', 'Cancel');
      popup.show();
    }
    ...
    K_new_clickListener() {
      ...
      if (this.workshop.unsaved) {
        var popup = new Attention.Confirmation({
          situation: 'information calm',
          title: 'Starting new workspace',
          message: 'You have unsaved changes in the workspace. Do you want to start a new one?',
          ok: { label: 'Yes', klass: 'flat' },
          cancel: { label: 'No', klass: 'flat' }
        });
        ...
        popup.show();
        popup.on('ok', function () {
          _this.K_new_clickListener_confirm();
        });
      }
      else {
        this.K_new_clickListener_confirm();
      }
    }
    ...
    K_saveAs_clickListener() {
      let panel = new SavePopup('Saving', 'Save', 'Cancel');
      panel.show();
    }

    
    // source: https://stackoverflow.com/a/26298948
    K_importFromFile_changeListener_confirm(evt) {
      let _this = this;
      ...
      reader.addEventListener('load', function (evt) {
        _this.addFromJSON(JSON.parse(evt.target.result));
      });
      reader.readAsText(file);
      K.gid('K_importFromFile').value = '';
    }

    K_importFromFile_changeListener(evt) {
      let _this = this;
      ...
      if (this.workshop.unsaved) {
        var popup = new Attention.Confirmation({
          situation: 'information calm',
          title: 'Importing workspace',
          message: 'You have unsaved changes in the workspace. Do you want to import a file?',
          ok: { label: 'Yes', klass: 'flat' },
          cancel: { label: 'No', klass: 'flat' }
        });
        ...
        popup
          .on('ok', function () {
            _this.K_importFromFile_changeListener_confirm(evt);
          })
          .on('dismiss', function () {
            K.gid('K_importFromFile').value = '';
          });
        ...
        this.K_importFromFile_changeListener_confirm(evt);
      }
    }
    ...
    K_document_cellLoadedListener(evt) {
      ...
      if (!Object.keys(this.imported.cells).length) {
        this.selectActiveCell(this.imported.active);
        this.selectActiveCube(this.imported.lastSelectedCube);
        this.restoreWorldPosition(this.imported.worldPosition);
      }
    }
    

    selectActiveCell(cellId) {
      workshop.list.makeActive(cellId);
    }

    selectActiveCube(cubeId) {
      tomni.getCurrentCell().setTarget(cubeId);
    }

    restoreWorldPosition(pos) {
      let camera = tomni.threeD.getCamera();

      camera.matrix.fromArray(JSON.parse(pos.cameraProps));
      camera.matrix.decompose(camera.position, camera.quaternion, camera.scale); 

      tomni.center.rotation.set(pos.tomniRotation.x, pos.tomniRotation.y, pos.tomniRotation.z);
      tomni.threeD.zoom = pos.threeDZoom;
  ...
  class List {
    constructor (workshop) {
      this.prefix = 'K_listEl_';
      this.workshop = workshop;
      this.emptyNode = '(empty)';
      this.counter = 0;

      this.node = document.createElement('div');
      this.node.id = 'K_listPanel';
      this.node.style.display = 'none';
      this.node.innerHTML = this.emptyNode;
      document.body.appendChild(this.node);

      this.node.addEventListener('click', this.K_listNode_clickListener.bind(this));
      this.node.addEventListener('input', this.K_listNode_inputListener.bind(this));
    }

    K_listNode_inputListener(evt) {
      ...
      if (evt.target.className === 'K_listColor') {
        this.workshop.changeCellColor(id, evt.target.value);
        evt.target.parentNode.style.backgroundColor = evt.target.value; 
      }
    }

    K_listNode_clickListener(evt) {
      let _this = this;
      ...
      switch (evt.target.className) {
        case 'K_listShow':
          tomni.threeD.getCell(id)[evt.target.checked ? 'show' : 'hide']();
          break;
        case 'K_listMakeActive':
          this.makeActive(id);
          break;
        case 'K_listBlink':
          let originalColor = this.workshop.cellsInUse[id].color;

          // done as a set of callbacks, to wait with blikning for downloading eventual missing cubes
          // and to now download them 4 times
          this.workshop.changeCellColor(id, '#ffffff', true, function () {
            setTimeout(function () {
              _this.workshop.changeCellColor(id, originalColor, true, function () {
                setTimeout(function () {
                  _this.workshop.changeCellColor(id, '#ffffff', true, function () {
                    setTimeout(function () {
                      _this.workshop.changeCellColor(id, originalColor, true);
                    }, 150);
                  });
                }, 150);
              });
        ...
        case 'K_listRemove':
          let popup = new Attention.Confirmation({
            situation: 'information calm',
            title: 'Removing cell',
            message: `Do you want to remove cell ${id}?`,
            ok: { label: 'Yes', klass: 'flat' },
            cancel: { label: 'No', klass: 'flat' }
          });
    ...
    makeActive(id) {
      tomni.cell = id;
      this.workshop.active = id;

      K.qSa('.K_listMakeActive').forEach( el => el.style.backgroundColor = '');
      K.gid(this.prefix + id).getElementsByClassName('K_listMakeActive')[0].style.backgroundColor = '#0d3fad';
    }
    ...
    addEl(args) {
      ...
      el.id = this.prefix + args.id;
      el.innerHTML = `
        <div class="K_listEl" data-id="${args.id}">
          <input type="checkbox" class="K_listShow" checked="checked" title="show/hide cell">
  ...
  class Popup {
    constructor (title = '', confirmText = 'OK', cancelText = 'Cancel') {
      let _this = this;

      if (!K.gid('K_popupBackground')) {
        this.background = document.createElement('div');
        this.background.id = 'K_popupBackground';

        this.wrapper = document.createElement('div');
        this.wrapper.id = 'K_popupWrapper';
        
        document.body.appendChild(this.background);
        document.body.appendChild(this.wrapper);
      }
      else {
        this.background = K.gid('K_popupBackground');
        this.wrapper = K.gid('K_popupWrapper');
      }
    
      this.wrapper.innerHTML = `
        <div id="K_popupTitleBar">${title}</div>
        <div id="K_popup"></div>
        <div id="K_popupFootBar">
          <div id="K_popupConfirmButton" class="K_popupButton">${confirmText}</div>
          <div id="K_popupCancelButton" class="K_popupButton">${cancelText}</div>
        </div>
      `;

      this.content = K.gid('K_popup');
      K.gid('K_popupConfirmButton').addEventListener('click', function () {
  ...
  class SavePopup extends Popup {
    constructor (title, confirmText, cancelText) {
      ...
      this.setContent(`
        ...
        <div class="K_popupRadioGroup" id="K_saveChooseTarget">
          ...
          <label><input type="radio" name="K_saveChooseTarget" value="server">on Server</label>
          <label><input type="radio" name="K_saveChooseTarget" value="file">to File</label>
        </div>
    ...
    K_saveChooseTarget_changeListener(evt) {
      if (evt.target.name !== 'K_saveChooseTarget') {
        return;
      }

      if (evt.target.value === 'server') {
        this.showRights();
      }
      else {
        this.hideRights();
    ...
    static saveToServer(data) {
      K.XHR({
        url: serverPath + 'save_workspace.php',
        method: 'POST',
        data: 'cells=' + JSON.stringify(data),
        success: function () {
          workshop.unsaved = false;
          workshop.workspace.target = 'server';
          workshop.workspace.rights = data.rights;
        }
      });
    }

    static saveToLocalStorage_confirm(cells, data) {
      cells[data.name] = data;
    ...
    static saveToLocalStorage(data) {
      ...
      if (cells[data.name]) {
        let popup = new Attention.Confirmation({
          ...
          message: `Entry named "${data.name}" already exists. Do you want to replace it?`,
          ok: { label: 'Yes', klass: 'flat' },
          cancel: { label: 'No', klass: 'flat' }
        });
        ...
        popup.show();
        popup.on('ok', function () {
          _this.saveToLocalStorage_confirm(cells, data);
        });
      }
      else {
        this.saveToLocalStorage_confirm(cells, data);
      }

      workshop.unsaved = false;
      workshop.workspace.target = 'localStorage';
    }
    ...
    static saveToFile(data) {
      ...
      // there's probably no way, to tell, if user actually saved the file,
      // or cancelled the Save Dialog, so we have to assume, that they did save it
      workshop.unsaved = false;
    }
    ...
    static getWorldPosition() {
      ...
      let cameraProps = JSON.stringify(camera.matrix.toArray());
      ...
      let threeDZoom = tomni.threeD.zoom;

      return {
        tomniRotation: tomniRotation,
        threeDZoom: threeDZoom,
    ...
    static save(name, target, description = '', rights = 0) {
      ...
      if (Object.keys(wholeCells).length === 0 && wholeCells.constructor === Object) {
        tomni.notificationManager.addChip({title: 'There\'s nothing to be saved'});
        return;
      }

      let cells = {};
      for (let cellId in wholeCells) {
        if (wholeCells.hasOwnProperty(cellId)) {
          let cell = wholeCells[cellId];
          cells[cellId] = { color: cell.color };
        }
      }
    ...
    confirmCallback() {
      ...
      let rights;
      if (target === 'server') {
        rights = K.qS('input[name=K_saveToServerAs]:checked').value;
      }
  ...
  class BrowsePopup extends Popup {
    constructor (title, confirmText, cancelText) {
      super(title, confirmText, cancelText);

      this.setContent(`
        <div class="K_popupRadioGroup" id="K_browseChooseSource">
          <div class="K_popupLabel">Source</div>
    ...
    K_browseChooseSource_changeListener(evt) {
      if (evt.target.name !== 'K_browseChooseSource') {
        return;
      }
    ...
    getEntriesFromLocalStorage(callback) {
      let obj = JSON.parse(K.ls.get('ewc'));
      let arr = [];
    ...
    fillList(source) {
      ...
      if (source === 'localStorage') {
        this.getEntriesFromLocalStorage(function (entries) {
          _this.fillListHelper(entries);
        });
      }
      else if (source === 'server') {
        this.getEntriesFromServer(function (entries) {
          _this.fillListHelper(entries);
        });
      }
    }
  ...
  class AddPopup extends Popup {
    constructor (title, confirmText, cancelText) {
      ...
      this.setContent(`
        <div id="K_addCellsAux">
          ...
          <div id="K_addCellsSelectedCellsIds">Selected cells: <span></span></div>
        </div>
        <table id="K_addCellsTable" width="100%"></table>
        <div id="K_loading">Loading...</div>
      `);
    ...
    updateCellsList() {
      let cells = this.getAllSelectedCells();

      this.selectedCells = cells;

      if (!cells.length) {
        K.gid('K_addCellsSelectedCellsIds').style.visibility = 'hidden';
      }
      else {
        K.gid('K_addCellsSelectedCellsIds').style.visibility = 'visible';
        K.qS('#K_addCellsSelectedCellsIds > span').innerHTML = cells.join(', ');
      }
    }

    getAllSelectedCells() {
      let cells = [];
      this.table.rows('.selected').every(function () {
        cells.push(this.data()[0]);
      });

      let manual = K.qS('#K_addCellsManual > input').value || '';

      // source: https://stackoverflow.com/a/23728809 & https://stackoverflow.com/a/21644539
      manual = manual.length ? manual.split(/[\s,]+/).map(num => parseInt(num, 10)).filter(Boolean) : [];
      cells = cells.map(num => parseInt(num, 10));

      // concatenate arrays and dedupe the result
      return cells = [...new Set([...cells, ...manual])];
    }
    ...
    fillTable() {
      ...
      this.table = $('#K_addCellsTable').DataTable({
        ...
        autoWidth: false,
        columns: [
          { title: 'ID', className: 'dt-body-right' },
          { title: 'Name' },
          { title: 'Size', className: 'dt-body-right' },
          { title: 'Description', width: 300 },
          { title: 'Status', className: 'dt-body-right' },
          { title: 'Creation Date', width: 80, className: 'dt-body-right' },
          { title: 'Completion Date', width: 80, className: 'dt-body-right' },
          { title: 'Dataset', width: 50, className: 'dt-body-right' }
        ],
        initComplete: function () {
          K.gid('K_loading').style.display = 'none';
          K.gid('K_addCellsManual').style.display = 'block';
        }
      });

      this.table.on('select deselect', function () {
        _this.updateCellsList();
      });
    }
      ...
      (function (open) {
        XMLHttpRequest.prototype.open = function (method, url, async, user, pass) {
          this.addEventListener("readystatechange", function (evt) {
            if (this.readyState == 4 && this.status == 200 && url.indexOf('/tasks') !== -1) {
              url = url.split('/');
              let cellId = parseInt(url[url.length - 2], 10);
